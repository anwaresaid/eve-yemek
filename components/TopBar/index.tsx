    ...
    return (
        <>
            <Card  id='main_context_card'className={"main-context-card" + (props.hideBar  ? "-showBar" : "-hideBar")} >
                <Button id='main_context_burger_btn' style={{ float:'left'}} icon={"pi " + (props.hideBar ? "pi-chevron-left" : "pi-chevron-right")} className=" p-button-danger" onClick={()=> { props.setHideBar(!props.hideBar) }}/>
                <div className='profileContainer'>
                    <div className='profile' onClick={toggleProfile}>
                        ...
                        <div className='menu'>
                            <h3>{i18n.t("EveYemek")}</h3>
                            <ul>
                                <li>{auth.user.email}</li>
                                <li>{auth.user.roles.length>1?auth.user.roles.map(d => d +', '):auth.user.roles}</li>
