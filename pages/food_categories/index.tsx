...
const FoodCategoriesList =  () => {
    ...
    const {loading, success, foodCat: foodCategoriesList} = res;
     ...
     useEffect( () => {
        ...
        if(success)
            setFoods(foodCategoriesList.items);
     }, [success]);
   ...
   const statusBodyTemplate = (rowData) => {
    ...
    const actionBodyTemplate = (rowData) => {
        return (
            <React.Fragment>
                <Button icon="pi pi-pencil" className="p-button-rounded p-button-success p-mr-2" onClick={()=>{router.push(`/food_categories/${rowData._id}`)}} />
            </React.Fragment>
        );
    }
...
// global filter
    const header =(
        <div className="table-header">
            ...
            <span className="p-input-icon-left">
                ...
                <InputText type="search" onInput={(e) => setGlobalFilter((e.target as HTMLInputElement).value)} placeholder="Ara" />
            </span>
        </div>
    )
    ...
    return(
        <>
            ...
            globalFilter={globalFilter} emptyMessage="Yemek kategorileri bulunamadı">
